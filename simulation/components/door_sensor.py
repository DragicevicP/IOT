...
def door_sensor_callback(state, system_info, mqtt_sender, device_id, simulated, event_bus=None):
    ...
    if event_bus is not None:
        event_bus.publish(DeviceEvent(
            device_id=device_id,
            value=state,          # 1=open, 0=closed (kao i do sada)
            simulated=simulated,
            timestamp=now_ts(),
...
def run_door_sensor(device_settings, threads, stop_event, mqtt_sender, system_info, event_bus=None):
    delay = device_settings.get("poll_interval", 2)
    device_id = device_settings["id"]
    simulated = is_simulated(device_settings)

    callback = partial(door_sensor_callback, system_info=system_info, mqtt_sender=mqtt_sender, device_id=device_id, simulated=simulated, event_bus=event_bus)
