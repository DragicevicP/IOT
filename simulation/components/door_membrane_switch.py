...
def dms_key_callback(key, system_info, mqtt_sender, device_id, simulated, state, event_bus):
    ...
    if key == '*':
        ...
        if entered == state["pin_code"]:
            ...
            # >>> OVO JE BITNO: šalješ PIN event logic engine-u
            if event_bus is not None:
                event_bus.publish(PinEvent(source_id="DMS", pin=entered, timestamp=now_ts()))
...
def run_door_membrane_switch(device_settings, threads, stop_event, mqtt_sender, system_info, event_bus=None):
    ...
    callback = partial(dms_key_callback, system_info=system_info, mqtt_sender=mqtt_sender, device_id=device_id, simulated=simulated, state=state, event_bus=event_bus)
